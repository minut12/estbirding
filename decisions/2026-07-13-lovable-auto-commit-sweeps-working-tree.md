...
- **Lovable's own self-reports are unreliable — three times in this one session:** a duplicate `const` block that would not compile, and two summaries that contradicted their own diffs. Combined with tree-sweeping auto-commit, the failure mode compounds: an agent can ship a broken edit, **misreport it as clean, and commit it before the diff is reviewed** — three independent failures lining up into one bad merge to `main`.
- **`get_diff` on every returned SHA is therefore mandatory, not optional.** It is the only gate that actually holds: it reads what was really committed, independent of the agent's self-report and independent of when the commit fired. Trust the diff, never the summary.
