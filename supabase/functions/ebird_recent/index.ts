...
Deno.serve(async (req) => {
  ...
  const token = Deno.env.get("EBIRD_API_TOKEN2");
  if (!token) {
    console.error("[ebird_recent] missing secret EBIRD_API_TOKEN2");
    return jsonResponse(500, { error: "EBIRD_API_TOKEN2 secret not set" }, origin);
  }
